...
if (length(adjusted_cols) == 0) {
  warning("NO DATA AVAILABLE FOR ANALYSIS IN THE PRE-COVID/PRE-ENERGY CRISIS PERIOD")
} else {
  ...
  if (nrow(merged_adjusted_data) < 2) {
    warning("NOT ENOUGH DATA AFTER REMOVING NA FOR THE PRE-COVID/PRE-ENERGY CRISIS PERIOD")
  } else {
    ...
    if (ncol(ftseData.ret) < 2) {
      warning("NOT AT LEAST TWO ASSETS FOR PORTFOLIO ANALYSIS IN THE PRE-COVID/PRE-ENERGY CRISIS PERIOD")
    } else {
      
      # ANALYZE RETURNS WITH DESCRIPTIVE STATISTICS
      cat("\nSTATISTICAL SUMMARY OF RETURNS (Pre-COVID/Pre-Energy Crisis):\n")
      print(summary(ftseData.ret))
      cat("\nMEAN RETURNS (Pre-COVID/Pre-Energy Crisis):\n")
      print(apply(ftseData.ret, 2, mean))
      cat("\nSTANDARD DEVIATION OF RETURNS (Pre-COVID/Pre-Energy Crisis):\n")
      ...
      print(returns_plot)
      ggsave("thesis/returns_pre_covid.png", plot = returns_plot, width = 10, height = 6, dpi = 300) # SAVE
      ...
      print(prices_plot)
      ggsave("thesis/prices_pre_covid.png", plot = prices_plot, width = 10, height = 6, dpi = 300) # SAVE
      
      # ANALYZE CORRELATIONS BETWEEN ASSET RETURNS
      cat("\nCORRELATION ANALYSIS (Pre-COVID/Pre-Energy Crisis):\n")
      png("thesis/correlations_image_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      assetsCorImagePlot(ftseData.ret)
      dev.off() 
      
      png("thesis/correlations_select_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      plot(assetsSelect(ftseData.ret))
      dev.off() 
      
      png("thesis/correlations_eigen_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      ...
      png("thesis/frontier_long_only_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      tailoredFrontierPlot(longFrontier, risk = "Cov")
      dev.off() 
      print(longFrontier)
      
      # VISUALIZE WEIGHTS AND RISK CONTRIBUTIONS OF THE LONG-ONLY PORTFOLIO
      num_assets <- ncol(ftseData.ret)
      col_palette <- seqPalette(num_assets, "YlOrRd")
      
      png("thesis/weights_long_only_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      weightsPlot(longFrontier, col = col_palette)
      dev.off() 
      
      png("thesis/returns_contributions_long_only_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      weightedReturnsPlot(longFrontier)
      dev.off() 
      
      png("thesis/risk_contributions_long_only_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      ...
      par(mfrow = c(1, 1))
      set.seed(1953)
      png("thesis/monte_carlo_simulation_long_only_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      ...
      # CALCULATION AND ANALYSIS OF SHARPE RATIO FOR LONG ONLY FRONTIER
      ...
      cat("\n------------------------------------------------\n")
      cat("SHARPE RATIO ANALYSIS - LONG ONLY FRONTIER (Pre-COVID/Pre-Energy Crisis)\n")
      cat("------------------------------------------------\n")
      ...
      png("thesis/sharpe_ratio_long_only_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      ...
      png("thesis/frontier_short_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      tailoredFrontierPlot(shortFrontier, risk = "Cov")
      dev.off() 
      print(shortFrontier)
      
      # VISUALIZE WEIGHTS AND RISK CONTRIBUTIONS OF THE SHORT PORTFOLIO
      png("thesis/weights_short_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      weightsPlot(shortFrontier, col = col_palette)
      dev.off() 
      
      png("thesis/returns_contributions_short_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      weightedReturnsPlot(shortFrontier)
      dev.off() 
      
      png("thesis/risk_contributions_short_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      ...
      par(mfrow = c(1, 1))
      set.seed(1953)
      png("thesis/monte_carlo_simulation_short_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
      ...
      # CALCULATION AND ANALYSIS OF SHARPE RATIO FOR SHORT FRONTIER
      ...
      cat("\n------------------------------------------------\n")
      cat("SHARPE RATIO ANALYSIS - SHORT FRONTIER (Pre-COVID/Pre-Energy Crisis)\n")
      cat("------------------------------------------------\n")
      ...
      png("thesis/sharpe_ratio_short_pre_covid.png", width = 800, height = 600, res = 100) # SAVE
...
cat("\n\n####################################################\n")
cat("### END ANALYSIS FOR PERIOD: Pre-COVID/Pre-Energy Crisis ###\n")
cat("####################################################\n\n")