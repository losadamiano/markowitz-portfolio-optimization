...
if (length(adjusted_cols) == 0) {
  warning("NO DATA AVAILABLE FOR ANALYSIS IN THE POST-COVID/WAR IN UKRAINE/INFLATION PERIOD")
} else {
  merged_adjusted_data <- do.call(merge, adjusted_cols)
  colnames(merged_adjusted_data) <- downloaded_tickers
  merged_adjusted_data <- na.omit(merged_adjusted_data)
  
  if (nrow(merged_adjusted_data) < 2) {
    warning("NOT ENOUGH DATA AFTER REMOVING NA FOR THE POST-COVID/WAR IN UKRAINE/INFLATION PERIOD")
  } else {
    ftseData <- as.timeSeries(merged_adjusted_data)
    ftseData.ret <- returns(ftseData) * 100 
    
    if (ncol(ftseData.ret) < 2) {
      warning("NOT AT LEAST TWO ASSETS FOR PORTFOLIO ANALYSIS IN THE POST-COVID/WAR IN UKRAINE/INFLATION PERIOD")
    } else {
      
      # ANALYZE RETURNS WITH DESCRIPTIVE STATISTICS
      cat("\nSTATISTICAL SUMMARY OF RETURNS (Post-COVID/War in Ukraine/Inflation):\n")
      print(summary(ftseData.ret))
      cat("\nMEAN RETURNS (Post-COVID/War in Ukraine/Inflation):\n")
      print(apply(ftseData.ret, 2, mean))
      cat("\nSTANDARD DEVIATION OF RETURNS (Post-COVID/War in Ukraine/Inflation):\n")
      ...
      print(returns_plot)
      ggsave("thesis/returns_post_covid_war_inflation.png", plot = returns_plot, width = 10, height = 6, dpi = 300) # SAVE
      ...
      print(prices_plot)
      ggsave("thesis/prices_post_covid_war_inflation.png", plot = prices_plot, width = 10, height = 6, dpi = 300) # SAVE
      
      # ANALYZE CORRELATIONS BETWEEN ASSET RETURNS
      cat("\nCORRELATION ANALYSIS (Post-COVID/War in Ukraine/Inflation):\n")
      png("thesis/correlations_image_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      assetsCorImagePlot(ftseData.ret)
      dev.off() 
      
      png("thesis/correlations_select_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      plot(assetsSelect(ftseData.ret))
      dev.off() 
      
      png("thesis/correlations_eigen_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      ...
      png("thesis/frontier_long_only_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      ...
      png("thesis/weights_long_only_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      weightsPlot(longFrontier, col = col_palette)
      dev.off()
      
      png("thesis/returns_contributions_long_only_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      weightedReturnsPlot(longFrontier)
      dev.off() 
      
      png("thesis/risk_contributions_long_only_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      ...
      par(mfrow = c(1, 1))
      set.seed(1953)
      png("thesis/monte_carlo_simulation_long_only_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      ...
      cat("\n------------------------------------------------\n")
      cat("SHARPE RATIO ANALYSIS - LONG ONLY FRONTIER (Post-COVID/War/Inflation)\n")
      cat("------------------------------------------------\n")
      ...
      png("thesis/sharpe_ratio_long_only_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      ...
      png("thesis/frontier_short_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      tailoredFrontierPlot(shortFrontier, risk = "Cov")
      dev.off() 
      print(shortFrontier)
      
      # VISUALIZE WEIGHTS AND RISK CONTRIBUTIONS (SHORT PORTFOLIO)
      png("thesis/weights_short_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      weightsPlot(shortFrontier, col = col_palette)
      dev.off() 
      
      png("thesis/returns_contributions_short_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      weightedReturnsPlot(shortFrontier)
      dev.off() 
      
      png("thesis/risk_contributions_short_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      ...
      par(mfrow = c(1, 1))
      set.seed(1953)
      png("thesis/monte_carlo_simulation_short_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
      ...
      cat("\n------------------------------------------------\n")
      cat("SHARPE RATIO ANALYSIS - SHORT FRONTIER (Post-COVID/War/Inflation)\n")
      cat("------------------------------------------------\n")
      ...
      png("thesis/sharpe_ratio_short_post_covid_war_inflation.png", width = 800, height = 600, res = 100) # SAVE
...
cat("\n\n####################################################\n")
cat("### END OF ANALYSIS FOR PERIOD: Post-COVID/War/Inflation ###\n")
cat("####################################################\n\n")